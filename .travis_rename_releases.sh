...
if [ -f "${GOPATH}/bin/tc-client-generator" ]; then
  mkdir "${GOPATH}/bin/linux_amd64"
  mv "${GOPATH}/bin/tc-client-generator" "${GOPATH}/bin/linux_amd64/tc-client-generator"
fi
...
export RELEASE_FILE="${TRAVIS_BUILD_DIR}/tc-client-generator-${GIMME_OS}-${GIMME_ARCH}${FILE_EXT}"
mv "${GOPATH}/bin/${GIMME_OS}_${GIMME_ARCH}/tc-client-generator${FILE_EXT}" "${RELEASE_FILE}"
