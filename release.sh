...
# This script is used to generate releases of the tc-client-generator. It
# should be the only way that releases are created. There are two phases, the
# first is checking that the code is in a clean and working state. The second
# phase is modifying files, tagging, commiting and pushing to github.

# exit in case of bad exit code
set -e

OFFICIAL_GIT_REPO='git@github.com:taskcluster/tc-client-generator'
...
OLD_VERSION="$(cat cmd/tc-client-generator/main.go | sed -n 's/^[[:space:]]*version *= *"\(.*\)"$/\1/p')"

VALID_FORMAT='^[0-9][0-9]*\.\(0\|[1-9][0-9]*\)\.\(0\|[1-9]\)\([0-9]*alpha[1-9][0-9]*\|[0-9]*\)$'
...
if ! echo "${OLD_VERSION}" | grep -q "${VALID_FORMAT}"; then
  echo "Previous release version '${OLD_VERSION}' not allowed (${FORMAT_EXPLANATION}) - please fix cmd/tc-client-generator/main.go" >&2
  exit 64
fi
...
inline_sed cmd/tc-client-generator/main.go 's/\(version *= *\)"'"${OLD_VERSION//./\\.}"'"$/\1"'"${NEW_VERSION}"'"/'
./refresh_readme.sh
git add README.md
